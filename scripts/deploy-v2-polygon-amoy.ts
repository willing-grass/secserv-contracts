...
async function main() {
  ...
  console.log("Deploying MessageMarketplaceV2 to Polygon Amoy...");
  console.log("Deployer address:", deployer.address);
  console.log("Account balance:", (await deployer.provider?.getBalance(deployer.address)).toString());

  // Configuration for Polygon Amoy
  const USDC_ADDRESS = "0x834bBE71a0a5C91A4Aff10Df400A963D95AD4775"; // Polygon Amoy test token
  ...
  const messageId = ethers.keccak256(ethers.toUtf8Bytes("test-message-polygon-amoy"));
  ...
  console.log("\nDeployment info saved to deployment-info-polygon-amoy.json");
  const fs = require("fs");
  fs.writeFileSync(
